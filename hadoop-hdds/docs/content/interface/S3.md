...
Setting `ozone.s3.signature.validation.enabled` to `true` narrows that on a cluster where
security is off: the OM then verifies the request signature against the secret stored for the
access key, and rejects a request signed with any other secret. It authenticates the credential
and nothing else, so Kerberos, certificates, block tokens, and ACLs all remain off and the access
key id stays an unauthenticated claim of identity everywhere else. It is meant for development
clusters and is not a substitute for enabling security.
